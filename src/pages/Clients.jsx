  ...
  return (
    <div className=" w-full lg:w-4/6  mx-auto  flex  flex-col lg:px-6 justify-around gap-10 py-6  ">
      ...
      <div className="w-full flex">
        {card.map((item, idx) => {
          return (
            <div key={idx} className="w-full flex flex-col gap-2  items-center px-6">
              <img src={avatar} alt="" />
      ...
      <div className="w-full  flex items-center justify-center flex-wrap  gap-5">
        {[1, 1, 1, 1, 1, 1, 1, 1].map((item, idx) => {
          return (
            <div key={idx} className="w-1/3 lg:w-1/5   flex justify-center p-5 border rounded-lg bg-white">
              <img src={client1} className="" />{item}
            </div>
          );
        })}
      </div>
